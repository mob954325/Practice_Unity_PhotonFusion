...
namespace Asteroids.HostSimple
{
    ...
    // 모든 메소드는 PlayerDataNetworked의 정보가 변경됨을 감지하면 호출된다.
    public class PlayerOverviewPanel : MonoBehaviour
    {
        // 한 플레이어의 정보를 표시하는 UI프리팹
        ...
        {
            if (_playerListEntries.ContainsKey(playerRef)) return;  // 이미 있는 사람을 추가하려하면 스킵
            if (playerDataNetworked == null) return;                // playerDataNetworked가 null이면 스킵

            var entry = Instantiate(_playerOverviewEntryPrefab, this.transform);    // 새 앤트리 생성
            entry.transform.localScale = Vector3.one;   // 스케일 초기화 -> 이유 모름
            entry.color = SpaceshipVisualController.GetColor(playerRef.PlayerId);   // 앤트리 표시 텍스트 색상 결정
            ...
            _playerListEntries.Add(playerRef, entry);   // 초기값으로 우선 항목만들기
        ...
        {
            if (_playerListEntries.TryGetValue(playerRef, out var entry) == false) return;  // 딕셔너리 없으면 스킵
        ...
        {
            if (_playerListEntries.TryGetValue(player, out var entry) == false) return; // 엔트리에 들어있는 플레이어 인지 확인

            _playerNickNames[player] = nickName;    // 갑 변경
