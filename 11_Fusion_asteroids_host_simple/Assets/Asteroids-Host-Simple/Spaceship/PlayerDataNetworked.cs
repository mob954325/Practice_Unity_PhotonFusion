...
namespace Asteroids.HostSimple
{
    ...
    {
        // 전역 스태틱 세팅
        private const int STARTING_LIVES = 3;   // 무조건 수명은 3개로 시작
        ...
        // [Networked] 파라미터가 변경되면 항상 OnChange함수로 넘어간다.
        [HideInInspector]
        [Networked]
        ...
        public override void Spawned()
        {
            ...
            {
                var nickName = FindObjectOfType<PlayerData>().GetNickName();    // PlayerData에서 플레이어의 이름을 가져오기
            ...
            _overviewPanel.AddEntry(Object.InputAuthority, this);       // 지금 접속한 플레이를 앤트리에 추가
        ...
        {
            _overviewPanel.RemoveEntry(Object.InputAuthority);  // overviewPanel에서 엔트리 제거
        }
        ...
        // 플레이어의 정보를 오너가 호스트에가 보내는 함수
        [Rpc(sources: RpcSources.InputAuthority, targets: RpcTargets.StateAuthority)]
        private void RpcSetNickName(string nickName)
        {
            if (string.IsNullOrEmpty(nickName)) return; // 빈 이름은 스킬
