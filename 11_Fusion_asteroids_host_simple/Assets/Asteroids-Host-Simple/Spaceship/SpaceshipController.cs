...
namespace Asteroids.HostSimple
{
    ...
    public class SpaceshipController : NetworkBehaviour
    {
        ...
        // 우주선과 충돌한 대상들에 대한 정보들(= 충돌한 운석들에 대한 정보)
        private List<LagCompensatedHit> _lagCompensatedHits = new List<LagCompensatedHit>();

        // 게임 세션 "상세" 세팅
        // 살아있고 오브젝트도 valid하면 true (리스폰 시간이나 게임 오버이후에 입력 접근 방지)
            ...
            foreach (var change in _changeDetector.DetectChanges(this, out var previousBuffer, out var currentBuffer))
            {
                switch (change)
                {
                    case nameof(_isAlive):  // _isAlive에 변화가 있는지 체크
                        var reader = GetPropertyReader<NetworkBool>(nameof(_isAlive));  // 값가져오기
        ...
        private void ToggleVisuals(bool wasAlive, bool isAlive)
        {
            ...
            if (wasAlive == false && isAlive == true)   // 이전에는 죽어있었는데 지금은 살아있을 경우
            {
                _visualController.TriggerSpawn();   // 비주얼 보이게 하기
            }
            // or whether it just got destroyed.
            else if (wasAlive == true && isAlive == false)  // 이전에 살아있었는데 지금은 죽어있는 경우
            {
                _visualController.TriggerDestruction(); // 비주얼 안보이게하기
            }
        }
        ...
        // 우주선이 운석과 직접 충돌 확인 (LagCompensation의 OverlapShphere를 이용해서 확인)
        private bool HasHitAsteroid()
        {
            ...
            var asteroidBehaviour = _lagCompensatedHits[0].GameObject.GetComponent<AsteroidBehaviour>();    // 가장 가까이의 있는 운석의 스크립트
            if (asteroidBehaviour.IsAlive == false) // 이미 터진 운석이면 종료
                return false;

            asteroidBehaviour.HitAsteroid(PlayerRef.None);  // 운석 파괴 처리 ( 몸으로 부딪치는 상황이니 점수를 안받기 위해서 PlayerRef.None를 파라미터로 넘김
        ...
        private void ShipWasHit()
        {
            ...
            if (_playerDataNetworked.Lives > 1) // 플레이어의 생명이 남아있다.
            {
                _respawnTimer = TickTimer.CreateFromSeconds(Runner, _respawnDelay); // 리스폰 타이머 작동
            }
            else // 생명이 없으면
            {
                _respawnTimer = default;        // 리스폰 타이머 없애기
            }

            _playerDataNetworked.SubtractLife();    // 플레이어의 생명 1 감소

            FindObjectOfType<GameStateController>().CheckIfGameHasEnded();  // 게임이 끝났는지 확인
        }
