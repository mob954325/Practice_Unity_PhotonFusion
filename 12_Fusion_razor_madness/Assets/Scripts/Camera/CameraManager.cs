...
public class CameraManager : MonoBehaviour
{
    ...
    public void SetSpectating()
    {
        ...
        CameraTarget = GetRandomSpectatingTarget(); // 내가 아닌 첫번째 반환
    }

    /// <summary>
    /// 자신이 아닌 관전 대상을 CameraTarget을 설정하기 (1명만 있을 때는 자기 자신)
    /// </summary>
    /// <returns></returns>
    ...
    private Transform GetNextOrPrevSpectatingTarget(int to)
    {
        ...
        {
            return _spectatingList[_spectatingList.Count - 1].transform.GetChild(0);    // 0이만이 되면 마지막으로 돌리기
        }
        else
        {
    ...
    private void Update()
    {
        if (_spectating)    // 관전 모드일 경우
        {
            ...
            {
                CameraTarget = GetNextOrPrevSpectatingTarget(1);    // 오른쪽 화살표를 누름녀 다음 캐릭터를 관전
            }
            else if (Input.GetKeyDown(KeyCode.LeftArrow))
            {
                CameraTarget = GetNextOrPrevSpectatingTarget(-1);   // 왼쪽 화살표를 누르면 이전 캐릭터 관전
            }
        }
    }
