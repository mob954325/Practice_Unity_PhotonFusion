...
public class LobbyCanvas : MonoBehaviour
{
    ...
    public void SetGameMode(int gameMode)
    {
        ...
        _gameMode = (GameMode)gameMode; // 눌러진 게임 모드 저장
        _modeButtons.SetActive(false);  // 모드 버튼 비활성화
        _nickname.transform.parent.gameObject.SetActive(true);  // 방 이름과 플레이어 이름을 설정할 수 있도록 활성화
    }

    // 플레이어 이름과 방 이름을 설정하고 아래 OK버튼을 눌렀을 때 실행하는 함수
    public void StartLauncher()
    {
        Launcher = FindObjectOfType<GameLauncher>();    // 런처 찾고
        Nickname = _nickname.text;                      
        PlayerPrefs.SetString("Nick", Nickname);        // 이름 저장
        Launcher.Launch(_gameMode, _room.text);         // 입력된 방이름과 게임 모드로 세션시작
