...
public class PlayerData: NetworkBehaviour
{
    ...
    public override void Spawned()
    {
        ...
        {
            string nickName = PlayerPrefs.GetString("Nick", string.Empty);  // 오너면 PlayerPrefs에서 값을 가져오기
        ...
        OnPlayerDataSpawnedEvent?.Raise(Object.InputAuthority, Runner); // OnPlayerDataSpawnedEvent에 추가되어 있는 델리게이트 설정

        if (Object.HasStateAuthority)
        {
            // 호스트라면 플레이어 레퍼런스와 이 게임 오브젝트를 연결하기
    ...
    public override void Render()
    {
        foreach (var change in _changeDetector.DetectChanges(this))
        {
            ...
            {
                case nameof(Nick):
                    OnPlayerDataSpawnedEvent?.Raise(Object.InputAuthority, Runner); // 이름이 바뀌면 OnPlayerDataSpwnedEvent 발생
                    break;
            }
        }
    }
