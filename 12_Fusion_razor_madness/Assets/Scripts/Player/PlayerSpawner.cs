...
public class PlayerSpawner : MonoBehaviour, INetworkRunnerCallbacks
{
    ...
    // 스폰되는 위치 (Respwan이라는 tag가 설정된 게임 오브젝트의 위치)
    public static Vector2 PlayerSpawnPos;
    
    private void Awake()
    {
        PlayerSpawnPos = GameObject.FindGameObjectWithTag("Respawn").transform.position; // 다른 리스폰 지점을 선정할 때 대비?
    ...
    {
        if (!runner.IsClient)   // 클라이언트가 아니면 -> HOST만 실행
        {
            PlayerSpawnPos = GameObject.FindGameObjectWithTag("Respawn").transform.position;    // 다시 스폰위치 찾기
            foreach (var player in runner.ActivePlayers)    // 러너에서 활성화 되있는 모든 플레이어에 대해
            {
                // 생성 (러너, 플레이어 레퍼런스, 플레이어 이름)
    ...
    private void SpawnPlayer(NetworkRunner runner, PlayerRef player, string nick = "")
    {
        if (runner.IsServer)    // 서버 (=호스트)만 실행
        {
            NetworkObject playerObj = runner.Spawn(
                ...
                player,                         // 플레이어 레퍼런스
                InitializeObjBeforeSpawn);      // 플레이어 오브젝트 초기화 함수 ( 스포전 실행 )
    ...
    {
        // obj = 생성은 되었지만 아직 네트워크 상에서 스폰은 안된 오브젝트
        var behaviour = obj.GetComponent<PlayerBehaviour>();
        // 플레이어 색상 랜덤 설정 (32비트 크기의 컬러로 만들기)
