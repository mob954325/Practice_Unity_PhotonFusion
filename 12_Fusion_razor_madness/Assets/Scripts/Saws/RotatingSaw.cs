...
{ 
    ...
    public override void Spawned()
    {
        ...
        _line = GetComponent<LineRenderer>();                   // 라인 랜더러 추가
        _renderer = GetComponentInChildren<SpriteRenderer>();   // 랜더러 찾기
        Origin = transform.position;                            // 자기 위치를 원점으로 설정
        Index = _randomStartAngle ? Random.Range(0, 360) : _startAngle; // 시작 각도 설정
        SetupLineRender();
    }
    ...
    {
        _line.positionCount = 2;        // 점 2개로 이뤄진 선
        _line.SetPosition(0, Origin);   // 중심점을 라인랜더러의 시작점으로 실행
    }

    public override void FixedUpdateNetwork()
    {
        transform.position = PointOnCircle(_radius, Index, Origin); // 원의 표면중 한 위치를 구하기
        Index = Index >= 360 ? 0 : Index + (1 * _speed);            // Index는 0 - 360
    }

    public override void Render()
    {
        _line.SetPosition(1, transform.position);   // 톱의 현재 위치를 라인 랜더러의 도착점으로 설정
        ...
        foreach (var changes in _changeDetector.DetectChanges(this))
        {
            ...
            {
                case nameof(Origin):
                    SetupLineRender();  // 톱의 위치가 변경되면 라인래더
                    break;
            }
        }
    }
